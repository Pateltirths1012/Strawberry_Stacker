...
class edrone_gripper():
    ...
    def __init__(self):
        ...
        rospy.init_node('edrone_0_service_server_gripper')
        ...
        self.gripper_service = rospy.Service('/edrone0/activate_gripper', Gripper, self.callback_service_on_request)
        ...
        if drone_index != -1:
            for box_model_name in self.box_model_name_list:
                try:
                    ...
                    box_index = -1
                if(box_index != -1):
                    #if(abs(dr_0 - bx_0) < 0.5 and abs(dr_1 - bx_1) < 0.5 and abs(bx_2 - dr_2) < 0.25):
                    if(abs(dr_0 - bx_0) < 0.5 and abs(dr_1 - bx_1) < 0.5 and 0 < dr_2 - bx_2 < 0.25):
                        pickable = True
